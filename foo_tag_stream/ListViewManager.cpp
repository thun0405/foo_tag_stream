...
{
    m_columns.push_back({ _T("#"), 30, [](int index, const TrackMetadata& track) { return pfc::string8(pfc::format_int(index + 1)); } });
    m_columns.push_back({ _T("Title"), 200, [](int index, const TrackMetadata& track) { return track.GetTitle(); } });
    m_columns.push_back({ _T("Artist"), 200, [](int index, const TrackMetadata& track) { return track.GetArtist(); } });
    m_columns.push_back({ _T("Album"), 200, [](int index, const TrackMetadata& track) { return track.GetAlbum(); } });
}
...
void ListViewManager::PopulateListView(const TrackMetadataList& tracks)
{
    for (size_t i = 0; i < tracks.GetCount(); ++i) {
        const TrackMetadata& track = tracks.GetAt(i);

        // Insert a new item for the first column
        const ColumnInfo& firstColumn = m_columns[0];
        pfc::string8 firstData = firstColumn.dataGetter(i, track);
        ...
        for (int j = 1; j < m_columns.size(); ++j) {
            ...
            pfc::string8 data = column.dataGetter(i, track);
